/**
 * @module app/shortcodes/wdpkCircle.js
 * @author Uneebox
 * @since Version 1.0.0
 * @returns object wdpkCircle
...
{
	// Declaring the wdpkCircle Object
	var wdpkCircle =
		{
			...
			loadCircle: function (obj)
			{
				...
				var waypoints = jQuery('#' + obj.selectorID).waypoint(function (direction)
				{
					...
					if (obj.valueProgress == true)
					{
						...
						{
							jQuery(this).find('.wdpk-cicles-counter-number').text(stepValue.toFixed(obj.decimalPlaces).substr(obj.valueTotal) + obj.suffixSymbol);
						});
					}
					...
					if (obj.animationProgress == true)
					{
						...
						{
							jQuery(this).find('.wdpk-cicles-counter-number').html(Math.round(100 * progress) + obj.suffixSymbol);
						});

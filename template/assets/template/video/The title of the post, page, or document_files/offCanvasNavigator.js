	...
	var offCanvasNavigator =
		{
			...
			navigator: function ()
			{
				...
				jQuery(".wdpk-hasDropdown, .wdpk-hasSubMenu .wdpk-dropDown-item").off("click touch").on("click touch", function (e)
				{
					...
					// A function to the find the container element inside the parent elemeent passed as a paramater
					//
					// @param parent - whether the element is parent or a child element, `true` for parent element and
					// `false` for the child element. @param identifier - Pass in the `id` of the parent element for
					// which you need the function to find the container
					//
					// @since version 1.0.0
					...
					if (currentActive == currentClicked && parentClicked == true)
					{
						...
						{
							findContainer(false, currentSubActive).removeClass('wdpk-offcanvas-active');
							jQuery("#" + currentSubActive).parent().find(".wdpk-offcanvas-plus").removeClass("wdpk-plus-transdform");
						}
						findContainer(true, currentClicked).toggleClass('wdpk-offcanvas-active');
					...
					{
						findContainer(false, currentSubClicked).removeClass('wdpk-offcanvas-active');
						jQuery(this).find(".wdpk-offcanvas-plus").removeClass("wdpk-plus-transdform");
					}
					else if (parentClicked == true && currentActive != currentClicked)
					{
						...
						{
							findContainer(true, currentActive).removeClass('wdpk-offcanvas-active');
							jQuery("#" + currentActive).parent().find(".wdpk-offcanvas-plus").removeClass("wdpk-plus-transdform");
						}
						...
						findContainer(true, currentClicked).addClass('wdpk-offcanvas-active');
					...
					else if (parentClicked == false && currentSubActive != currentSubClicked)
					{
						...
						{
							findContainer(true, currentSubActive).removeClass('wdpk-offcanvas-active');
							jQuery("#" + currentSubActive).parent().find(".wdpk-offcanvas-plus").removeClass("wdpk-plus-transdform");
						}
						findContainer(false, currentSubClicked).addClass('wdpk-offcanvas-active');
						jQuery(this).find(".wdpk-offcanvas-plus").addClass("wdpk-plus-transdform");
